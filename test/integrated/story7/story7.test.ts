...
describe('Story 7 - Integration Test: View approved reports on interactive map', () => {
  ...
  afterAll(async () => {
    ...
    if (approvedReportId) {
      await prisma.report.delete({ where: { id: approvedReportId } }).catch(() => {});
    }
    if (pendingReportId) {
      await prisma.report.delete({ where: { id: pendingReportId } }).catch(() => {});
    }
    if (testPhotoId) {
      await prisma.photo.delete({ where: { id: testPhotoId } }).catch(() => {});
    }
    if (testOfficerId) {
      await prisma.user.delete({ where: { id: testOfficerId } }).catch(() => {});
    }
    if (testCitizenId) {
  ...
  describe('Report Details Retrieval Flow', () => {
    it('should return full details of an approved report including photos', async () => {
      ...
      if (result.data) {
        ...
        expect(result.data.description).toBe('This is an approved report that should appear on the map');
    ...
    it('should retrieve report without officer assignment', async () => {
      ...
      const result = await getReportById({ id: reportWithoutOfficer.id.toString() });
    ...
    it('should retrieve anonymous reports correctly', async () => {
      ...
      if (anonymousUser) {
        ...
        const result = await getReportById({ id: anonymousReport.id.toString() });
