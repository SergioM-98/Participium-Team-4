      ...
      <div className="flex flex-col md:flex-row items-stretch gap-4 p-4 md:p-6 overflow-hidden flex-1 min-h-0">
        ...
        <div className="flex-[1.3] md:flex-1 min-h-0 rounded-lg border border-border bg-muted/10 overflow-hidden flex flex-col">
          ...
          <div className="flex-1 min-h-0 relative">
            {/* CASE 1: activeTab === "main" && canViewChat */}
            {activeTab === "main" && canViewChat && (
              <ChatPanel
                reportId={report.id}
            ...
            {/* CASE 2: activeTab === "main" && !canViewChat && isOfficerMode */}
            {activeTab === "main" && !canViewChat && isOfficerMode && (
            ...
            {/* CASE 3: activeTab === "main" && !canViewChat && !isOfficerMode && isMaintainerMode */}
            {activeTab === "main" && !canViewChat && !isOfficerMode && isMaintainerMode && (
              <div className="w-full h-full overflow-auto p-3">
                {/* MaintainerActionPanel placeholder */}
              </div>
            )}

            {/* CASE 4: activeTab === "main" && !canViewChat && !isOfficerMode && !isMaintainerMode */}
            {activeTab === "main" && !canViewChat && !isOfficerMode && !isMaintainerMode && (
            ...
            {/* CASE 5: activeTab === "internal" && (TECHNICAL_OFFICER || EXTERNAL_MAINTAINER_WITH_ACCESS) */}
            {activeTab === "internal" && currentUserRole === "TECHNICAL_OFFICER" && (
              <InternalNotesPanel reportId={report.id} />
            )}

            {/* CASE 6: activeTab === "internal" && EXTERNAL_MAINTAINER_WITH_ACCESS */}
            {activeTab === "internal" && currentUserRole === "EXTERNAL_MAINTAINER_WITH_ACCESS" && (
              <InternalNotesPanel reportId={report.id} />
            )}
          </div>
        </div>
