...
describe("ReportRepository Story 4", () => {
  ...
  describe("assignReportToOfficer - Story 6", () => {
    it("should assign report to officer successfully", async () => {
      ...
      mockedPrisma.report.update = jest
        .fn()
        .mockResolvedValue(mockUpdatedReport);
  ...
  describe("rejectReport - Story 6", () => {
    it("should reject report with reason successfully", async () => {
      ...
      mockedPrisma.report.update = jest
        .fn()
        .mockResolvedValue(mockRejectedReport);
  ...
  describe("getReportsByMaintainerId", () => {
    it("should retrieve reports for a specific maintainer", async () => {
      ...
      expect(mockedPrisma.report.findMany).toHaveBeenCalledWith({
        ...
        include: expect.anything(), // or specify the full include object
      });
    ...
    it("should throw error if db fails", async () => {
      mockedPrisma.report.findMany = jest
        .fn()
        .mockRejectedValue(new Error("DB Error"));
      await expect(
        reportRepository.getReportsByMaintainerId("m1")
