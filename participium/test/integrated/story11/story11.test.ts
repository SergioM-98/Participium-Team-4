...
import { getReportMessages, sendMessage } from "../../../src/app/lib/controllers/message.controller";
import { getInbox, getUnreadCount, markAllAsRead, markAsRead } from "../../../src/app/lib/controllers/notification.controller";
  ...
  afterAll(async () => {
    ...
    it("should retrieve the user notifications", async ()=> {
        ...
        if (notifications.success && notifications.data) {
            ...
            expect(messages).toContain("Your report status has been updated to: APPROVED");
            expect(messages).toContain("officer-123 sent you a new message on your report");
        }
    });
